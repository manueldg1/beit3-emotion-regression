...
class Pooler(nn.Module):
    def __init__(self, input_features, output_features, norm_layer):
        super().__init__()
        # Normalization layer to stabilize the token [CLS]
        self.norm = norm_layer(input_features)
        # Linear projection to turn the features for the specific task
        self.dense = nn.Linear(input_features, output_features)
        # Apply the non-linear activation function GELU
        self.activation = nn.GELU()

    def forward(self, x):
        # Extract the first token of the sequence [CLS]
        cls_rep = x[:, 0, :] # [Batch, Seq_len, Dim]
...
class BEiT3ForValenceArousalRegression(BEiT3Wrapper):
    def __init__(
        ...
        **kwargs
        ...
        # Dimensionality bottleneck: Adding a hidden layer to reduce dimensionality progressively
        if hidden_features is None:
            hidden_features = embed_dim // 4

        # 1. Pooler initialization
        self.pooler = Pooler(
            input_features=embed_dim, # defined in modeling_utils.py, 768 for base and 1024 for large
            output_features=embed_dim, 
            norm_layer=norm_layer, 
        )
        # Initialize the pooler sub-modules using the _init_weights logic from modeling_utils.py
        self.pooler.apply(self._init_weights)
        
        # 2. Valence Head based on TwoLayerMLP architecture
        self.valence_mlp = TwoLayerMLP(
            in_features=embed_dim, 
            hidden_features=hidden_features, 
            out_features=1, # Output is a single Valence score
            norm_layer=norm_layer,
            norm_input=False, # Disabled because the Pooler already applies a GELU function 
        )
        self.valence_mlp.apply(self._init_weights)

        # 3. Arousal Head based on TwoLayerMLP architecture
        ...
        # Shrink initial weights to ensure the model make early predictions closed to zero
        # At the beginning our model know nothing about emotions and first predictions are random extreme values.
        # Scaling them down prevents loss from exploding during the first steps,
        # resulting in more stable training and cleaner loss curves for analysis.
        init_scale = 0.001
        with torch.no_grad():
            for mlp in [self.valence_mlp, self.arousal_mlp]:
                for m in mlp.modules():
                    if isinstance(m, nn.Linear):
                        m.weight.mul_(init_scale)
                        m.bias.mul_(init_scale)

        # 5. FROZEN EMBEDDINGS (Ablation Study)
        # Verify if freeze_embeddings is True or if is set as args
        should_freeze = freeze_embeddings or getattr(args, "freeze_embeddings", False)

        if should_freeze:
            print("\n[ABLATION STUDY] Congelamento degli Embedding Layer di BEiT-3...")
            frozen_count = 0
            for name, param in self.beit3.named_parameters():
                # Congela sia l'embedding del testo (text_embed) che quello visivo (vision_embed/patch_embed)
                if "embed" in name:
                    param.requires_grad = False
                    frozen_count += param.numel()
                    print(f" -> Congelato: self.beit3.{name}")
            
            print(f"Totale parametri embedding congelati: {frozen_count:,}\n")



    def get_num_layers(self):
        """
        Retrieves the number of layers in the BEiT-3 encoder.
        Required to make BEiT3Wrapper compatible with standard training pipelines.
        """
        return self.beit3.encoder.num_layers
    
    

    def forward(self, image=None, text_description=None, padding_mask=None, **kwargs):
        ...
        v = self.final_activation(self.valence_mlp(cls))
        a = self.final_activation(self.arousal_mlp(cls))

        return torch.cat([v, a], dim=-1), cls
...
@register_model
def beit3_base_patch16_224_valence_arousal(pretrained=False, **kwargs):
...
@register_model
def beit3_large_patch16_480_valence_arousal_native_vocab(pretrained=False, **kwargs):
    """Original_vqa ablation: native BEiT-3 vocab_size (64010), no
    multilingual adaptation."""
    if 'vocab_size' in kwargs:
        kwargs.pop('vocab_size')
    args = _get_large_config(img_size=480, vocab_size=64010, **kwargs)
    args.normalize_output = False
    model = BEiT3ForValenceArousalRegression(args, **kwargs)
    return model
