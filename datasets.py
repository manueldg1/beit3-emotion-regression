...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _get_image(self, image_path: str):
        ...
        # If the path in JSONL is already absolute, don't prepend data_path
    ...
    def _get_text_segment(self, text):
        ...
        # Get max sequence length from dataset attributes
        max_len = self.num_max_bpe_tokens 

        # Add special tokens (BOS/EOS) and apply truncation
        # Format: [BOS] + tokens + [EOS]
        token_ids = [self.bos_token_id] + token_ids[:max_len - 2] + [self.eos_token_id]
        num_tokens = len(token_ids)
        
        # Create attention mask (0 for tokens, 1 for padding)
        padding_mask = [0] * num_tokens + [1] * (max_len - num_tokens)
        
        # Final sequence with padding tokens
        full_token_ids = token_ids + [self.pad_token_id] * (max_len - num_tokens)
        
        return full_token_ids, padding_mask, num_tokens
    ...
    def _get_image_text_example(self, index: int, data: dict):
        ...
        if img_path and str(img_path).strip():
            try:
                data["image"] = self._get_image(img_path)
            except Exception:
                # Use a black image if the file is missing or corrupted
                data["image"] = torch.zeros(3, self.args.input_size, self.args.input_size)
        else:
            # Handle text-only samples with a black placeholder image of correct size
            data["image"] = torch.zeros(3, self.args.input_size, self.args.input_size) 

        # Text processing
        text = item.get("Text")
        # Handle empty text with the placeholder logic we discussed
        if not text or not str(text).strip():
            text = "."
        
        # This calls the fixed _get_text_segment (the one with IDs, not strings)
        language_tokens, padding_mask, _ = self._get_text_segment(str(text))
        data["language_tokens"] = torch.tensor(language_tokens, dtype=torch.long)
        data["padding_mask"] = torch.tensor(padding_mask, dtype=torch.long)

        # Label processing
        # Convert VA labels to float and handle missing values with defaults
        v = float(item.get("Valence", 0.0))
        a = float(item.get("Arousal", 0.0))
        data["label"] = torch.tensor([v, a], dtype=torch.float32)

    def __getitem__(self, index: int):
        data = dict()
        # This function will fill the dictionary
...
class VARegressionDataset(BaseDataset):
    ...
    @staticmethod
    def get_index_files(split, task=None):
        # Define the specific JSONL files for the emotion recognition task
        # Using 10% ablation files for faster experimentation
        files = {
            "train": ["ablation_train_10pct.jsonl"],
            "val": ["ablation_val_10pct.jsonl"],
            "test": ["ablation_test_10pct.jsonl"]
        }
        # Return the specific file for the split, or a default name if not found
        return files.get(split, [f"{split}.jsonl"])
