...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='beit3_large_patch16_480', type=str, metavar='MODEL',
    ...
    # Finetuning params
    parser.add_argument('--finetune', default='./beit3_xlmr_final_ready.pt', type=str,
...
def main(args, ds_init):
    ...
    # To enhance multilingual capabilities replace BEiT-3 text embeddings  
    # with weights from XLM-R, previously projected into the BEiT-3 latent space.
    xlmr_path = "xlmr_in_beit3_space.pt"
    if os.path.exists(xlmr_path):
        print(f"Loading mapped XLM-R embeddings from {xlmr_path}")
        xlmr_weights = torch.load(xlmr_path, map_location='cpu')
        model.beit3.text_embed.weight.data.copy_(xlmr_weights)
    ...
    if args.eval:
        ...
        evaluate(data_loader_test, model, device, task_handler)
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            # Best-model checkpointing logic based on the lowest Root Mean Square Error (RMSE).
