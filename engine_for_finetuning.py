...
class VARegressionHandler(TaskHandler):
    """
    Handler for Valence and Arousal (VA) regression tasks.
    This class manages the training and evaluation logic for emotion recognition,
    allowing the comparison of 5 different loss functions.
    """
    def __init__(self, args) -> None:
        ...
        # Define the device forcing cuda:0 for robust-loss compatibility
        if torch.cuda.is_available():
            self.device = 'cuda:0'
        else:
            self.device = 'cpu'

        # Initialize CCC metric of torchmetrics
        self.ccc_metric = ConcordanceCorrCoef(num_outputs=2).to(self.device)

        # Sets the loss function based on the --loss_type command-line argument.
        # Defaults to ccc if no argument is provided.
        self.loss_type = getattr(args, 'loss_type', 'ccc').lower() # case insensitive
        
        # Initialize the Robust Adaptive Loss only if selected to optimize memory
        # This loss learns the distribution of noise in the data during training.
        if 'robust' in self.loss_type:
    ...
    def compute_ccc_loss(self, pred, target):
        """
        ...
        pv_var, tv_var = torch.var(pv, unbiased=False), torch.var(tv, unbiased=False)
    ...
    def compute_metrics(self, y_pred, y_true, update_ccc_state=False):
        """
        Computes standard regression metrics to assess model performance:
        RMSE, MAE, and Pearson Correlation Coefficient (rho).
        Metrics are calculated separately for Valence (V) and Arousal (A).
        """
        metrics = {}

        if update_ccc_state:
            # Update the shared stateful accumulator — called once per
            # epoch in after_eval() on the full concatenated predictions.
            ccc_values = self.ccc_metric(
                y_pred.to(self.device),
                y_true.to(self.device)
            )
        else:
            # Create a temporary stateless CCC instance for batch-level
            # logging during training. Does NOT affect self.ccc_metric.
            tmp_ccc = ConcordanceCorrCoef(num_outputs=2).to(self.device)
            with torch.no_grad():
                ccc_values = tmp_ccc(
                    y_pred.to(self.device),
                    y_true.to(self.device)
        ...
        # CCC per dimension
        metrics['v_ccc'] = ccc_values[0]
        metrics['a_ccc'] = ccc_values[1]
 
        # Pearson, RMSE, MAE — all stateless, safe to call anywhere
        for i, name in enumerate(['v', 'a']):
            p = y_pred[:, i]
            t = y_true[:, i]
 
            # Pearson Correlation Coefficient (rho)
            # Formula: cov(p, t) / (std(p) * std(t))
            p_diff      = p - torch.mean(p)
            t_diff      = t - torch.mean(t)
            numerator   = torch.sum(p_diff * t_diff)
            denominator = torch.sqrt(
                torch.sum(p_diff ** 2) * torch.sum(t_diff ** 2)
            ) + 1e-8   # epsilon for numerical stability
            metrics[f'{name}_pearson'] = numerator / denominator
 
            # RMSE — Root Mean Squared Error
            metrics[f'{name}_rmse'] = torch.sqrt(F.mse_loss(p, t))
 
            # MAE — Mean Absolute Error
            metrics[f'{name}_mae'] = F.l1_loss(p, t)
 
        return metrics
 

    def train_batch(self, model, image=None, text_description=None, padding_mask=None, label=None, **kwargs):
        ...
        if self.loss_type == "mse":
            ...
            # CCC loss: 1 - CCC, minimized when predictions agree with labels
            loss_v, loss_a = self.compute_ccc_loss(logits, label)
            loss = (loss_v + loss_a) / 2

        elif self.loss_type == "robust":
            # Robust adaptive loss — learns noise distribution during training
            loss = torch.mean(self.adaptive.lossfun(label - logits))

        elif self.loss_type == "mse_ccc":
            # Combined loss: equal weighting of MSE and CCC
            loss_v, loss_a = self.compute_ccc_loss(logits, label)
            ccc_loss = (loss_v + loss_a) / 2
            loss = 0.5 * F.mse_loss(logits, label) + 0.5 * ccc_loss

        elif self.loss_type == "robust_ccc":
            # Combined loss: equal weighting of Robust and CCC
            loss_v, loss_a = self.compute_ccc_loss(logits, label)
            ccc_loss = (loss_v + loss_a) / 2
            robust   = torch.mean(self.adaptive.lossfun(label - logits))
            loss = 0.5 * robust + 0.5 * ccc_loss

        else:
            # Fallback default: CCC loss
            loss_v, loss_a = self.compute_ccc_loss(logits, label)
            loss = (loss_v + loss_a) / 2
    ...
    def eval_batch(self, model, image=None, text_description=None, padding_mask=None, label=None, metadata=None, **kwargs):

        if image is not None:
            image = image.to(torch.float32)

        if text_description is not None:
            text_description = text_description.long()
        ...
        if isinstance(output, tuple):
            logits, embeddings = output[0], output[1]
        else:
            logits = output
            embeddings = None

        self.all_preds.append(logits.detach().cpu())
        self.all_labels.append(label.detach().cpu())
        ...
        rmse_v = torch.sqrt(F.mse_loss(logits[:, 0], label[:, 0]))
        rmse_a = torch.sqrt(F.mse_loss(logits[:, 1], label[:, 1]))

        self.metric_logger.update(rmse_v=rmse_v.item())
        self.metric_logger.update(rmse_a=rmse_a.item())

        return rmse_v + rmse_a, batch_size
   


    def after_eval(self, **kwargs):
        # Aggregate predictions and labels
        all_preds_cat  = torch.cat(self.all_preds,  dim=0)
        all_labels_cat = torch.cat(self.all_labels, dim=0)

        # Compute epoch-level metrics
        stats    = self.compute_metrics(all_preds_cat, all_labels_cat,
                                        update_ccc_state=True)
        sum_rmse = stats['v_rmse'] + stats['a_rmse']
        self.ccc_metric.reset()

        # Convert tensors to scalars
        stats = {k: v.item() if torch.is_tensor(v) else v
                 for k, v in stats.items()}
        stats['sum_rmse'] = sum_rmse.item() if torch.is_tensor(sum_rmse) \
                            else sum_rmse

        # Print results
        print("\n" + "=" * 100)
        print("--- FINAL EPOCH EVALUATION RESULTS ---")
        print("-" * 100)
        print(f"VALENCE | Pearson: {stats['v_pearson']:.4f} | "
              f"CCC: {stats['v_ccc']:.4f} | "
              f"RMSE: {stats['v_rmse']:.4f} | "
              f"MAE: {stats['v_mae']:.4f}")
        print(f"AROUSAL | Pearson: {stats['a_pearson']:.4f} | "
              f"CCC: {stats['a_ccc']:.4f} | "
              f"RMSE: {stats['a_rmse']:.4f} | "
              f"MAE: {stats['a_mae']:.4f}")
        print("-" * 100)
        print(f"SUM RMSE: {stats['sum_rmse']:.4f}  (model selection criterion)")
        print("=" * 100 + "\n")

        # Save JSONL with predictions + embeddings for OT-CP+
        if self.args is not None and len(self.all_metadata) > 0:
            import os
            out_dir       = self.args.output_dir
            test_file_arg = getattr(self.args, 'test_file', 'eval_results.jsonl')
            if not test_file_arg:
                test_file_arg = 'eval_results.jsonl'
            test_stem = os.path.splitext(os.path.basename(test_file_arg))[0]
            out_path  = os.path.join(out_dir,
                                     f"{test_stem}_with_predictions.jsonl")
            os.makedirs(out_dir, exist_ok=True)

            # Flatten list-of-batch-metadata
            flat_meta = []
            for batch_meta in self.all_metadata:
                n_batch = len(next(iter(batch_meta.values())))
                for i in range(n_batch):
                    flat_meta.append({k: v[i]
            ...
            if self.all_embeddings:
                ...
                all_embs_cat = torch.cat(self.all_embeddings, dim=0)
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                for i, meta in enumerate(flat_meta):
                    row = {
                        ...
                        "Arousal":      float(meta.get("Arousal", 0)),
                        "Pred_Valence": float(all_preds_cat[i, 0]),
                        "Pred_Arousal": float(all_preds_cat[i, 1]),
                    }
                    if all_embs_cat is not None:
                        row["Embedding"] = all_embs_cat[i].tolist()
                    f.write(json.dumps(row, ensure_ascii=False) + "\n")

            print(f"[after_eval] Saved {len(flat_meta)} samples -> {out_path}")
...
def get_handler(args):
    if args.task == "va_regression":
        return VARegressionHandler(args)
...
def train_one_epoch(
        ...
        args = None, model_without_ddp = None,
):
    model.train(True)
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        if data is None:
            # Entire batch was dropped by custom_collate_fn (all images missing/corrupted)
            print(f"[WARNING] Skipping empty batch at step {data_iter_step} (epoch {epoch})")
            continue
        ...
        if lr_schedule_values is not None and data_iter_step % update_freq == 0:
            for i, param_group in enumerate(optimizer.param_groups):
                if lr_schedule_values is not None:
                    param_group["lr"] = lr_schedule_values[min(global_step, len(lr_schedule_values) - 1)] * param_group["lr_scale"]

        # put input data into cuda
        for tensor_key in data.keys():
            if data[tensor_key] is not None:
                if isinstance(data[tensor_key], torch.Tensor):
                    data[tensor_key] = data[tensor_key].to(device, non_blocking=True)
                elif isinstance(data[tensor_key], list):
                    # Gestione per liste di tensori (se usate nel collate)
                    data[tensor_key] = [t.to(device, non_blocking=True) if t is not None else None for t in data[tensor_key]]
        ...
        if args is not None and args.save_ckpt_freq > 0 and (data_iter_step + 1) % update_freq == 0:
            if (global_step + 1) % args.save_ckpt_freq == 0:
                print(f"Forcing checkpoint saving at global step {global_step + 1}")
                utils.save_model(
                    args=args, 
                    epoch=epoch,  
                    model=model, 
...
@torch.no_grad()
def evaluate(data_loader, model, device, handler, args=None):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
