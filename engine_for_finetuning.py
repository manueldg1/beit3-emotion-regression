...
class VARegressionHandler(TaskHandler):
    ...
    def compute_metrics(self, y_pred, y_true):
        ...
        ccc_values = self.ccc_metric(y_pred.to(self.device), y_true.to(self.device))
        metrics['v_ccc'] = ccc_values[0]
        metrics['a_ccc'] = ccc_values[1]

        # Iterate through each emotional dimension
        for i, name in enumerate(['v', 'a']):
            ...
            # Numerator: sum of products of differences (covariance numerator)
            numerator = torch.sum(p_diff * t_diff)
            # Denominator: product of the standard deviations
            denominator = torch.sqrt(torch.sum(p_diff**2) * torch.sum(t_diff**2)) + 1e-8
            # Pearson Correlation Coefficient (rho)
            pearson_dim = numerator / denominator

            # Error-based metrics
            # Compute MSE necessary to get RMSE
            mse_dim = F.mse_loss(p, t)
            # RMSE (Root Mean Squared Error)
            rmse_dim = torch.sqrt(mse_dim)

            # MAE (Meand Absolute Error)
            mae_dim = F.l1_loss(p, t)

            # Store results in the metrics dictionary
            metrics[f'{name}_pearson'] = pearson_dim
            metrics[f'{name}_rmse'] = rmse_dim
            metrics[f'{name}_mae'] = mae_dim

        return metrics


    def train_batch(self, model, image, language_tokens, padding_mask, label, **kwargs):
        """
        Executes a single training step. It selects the loss function
        specified in the config to calculate gradients.
        """
        # Forward pass through the BEiT-3 model
        logits = model(image=image, text_description=language_tokens, padding_mask=padding_mask)
        label = label.float()
        ...
        if self.loss_type == 'mse':
            ...
            loss = self.compute_ccc_loss(logits, label)

        elif self.loss_type == 'robust':
            # Adaptive loss that handles outliers automatically
            residuals = label - logits
            loss = torch.mean(self.adaptive.lossfun(residuals))
        ...
        # Performance evaluation without gradient tracking
        with torch.no_grad():
            output_metrics = self.compute_metrics(logits, label)

        # Re-attach the loss tensor for backpropagation
        output_metrics["loss"] = loss

        return output_metrics


    def eval_batch(self, model, image, language_tokens, padding_mask, label, **kwargs):
        """
        Performs an evaluation step.
        We use torch.no_grad() to save memory and compute local batch metrics.
        """
        logits = model(image=image, text_description=language_tokens, padding_mask=padding_mask)
        label = label.float()

        # We detach and move to CPU to avoid GPU memory accumulation during the eval loop.
        # Storing as numpy arrays is usually more memory-efficient for large datasets.
        self.all_preds.append(logits.detach().cpu())
        self.all_labels.append(label.detach().cpu())

        batch_size = image.shape[0]

        # RMSE as batch-level metric
    ...
    def after_eval(self, **kwargs):
        """
        Finalizes the evaluation epoch. 
        Calculates global metrics across the entire validation set.
        """
        # Aggregate all batch predictions and labels into global tensors
        preds = torch.cat(self.all_preds, dim=0)
        labels = torch.cat(self.all_labels, dim=0)

        # Compute global epoch metrics (Pearson, CCC, RMSE, MAE)
        epoch_global_metrics = self.compute_metrics(preds, labels)
        
        # Convert tensors to scalar floats for logging and printing
        epoch_global_metrics = {k: v.item() if torch.is_tensor(v) else v for k, v in epoch_global_metrics.items()}

        # Reset CCC state for the next evaluation cycle
        self.ccc_metric.reset()

        # Display epoch results
        print(f"\n" + "="*100)
        print(f"--- FINAL EPOCH EVALUATION RESULTS ---")
        print(f"-"*100)
        # Log metrics for Valence and Arousal dimensions
        print(f"VALENCE | Pearson: {epoch_global_metrics['v_pearson']:.4f} | CCC: {epoch_global_metrics['v_ccc']:.4f} | RMSE: {epoch_global_metrics['v_rmse']:.4f} | MAE: {epoch_global_metrics['v_mae']:.4f}")
        print(f"AROUSAL | Pearson: {epoch_global_metrics['a_pearson']:.4f} | CCC: {epoch_global_metrics['a_ccc']:.4f} | RMSE: {epoch_global_metrics['a_rmse']:.4f} | MAE: {epoch_global_metrics['a_mae']:.4f}")

        # Stopping Criterion: The model will save the checkpoint based on the lowest sum of RMSEs
        sum_rmse = epoch_global_metrics['v_rmse'] + epoch_global_metrics['a_rmse']
        print(f"-"*100)
        print(f"SUM RMSE: {sum_rmse:.4f} (Stopping Criterion)")
        print(f"="*100 + "\n")

        # Update global statistics logger
        stats = {k: meter.global_avg for k, meter in self.metric_logger.meters.items()}
        stats.update(epoch_global_metrics)
        stats['sum_rmse'] = sum_rmse

        # Clear memory buffers for the next epoch
