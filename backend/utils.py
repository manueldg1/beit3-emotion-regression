    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            if 'optimizer' in checkpoint and 'epoch' in checkpoint:
                ...
                try:
                    if isinstance(checkpoint['epoch'], str) and checkpoint['epoch'] == 'best':
                        args.start_epoch = 4  # Forza l'inizio dall'epoca 4
                        print("Detected 'best' checkpoint. Manually setting start_epoch to 4.")
                    else:
                        args.start_epoch = int(checkpoint['epoch']) + 1
                except Exception as e:
                    print(f"Error parsing epoch: {e}. Defaulting start_epoch to 0.")
                    args.start_epoch = 0
        ...
        # Se auto_resume è attivo e non è stato specificato un file preciso
        if args.auto_resume and len(args.resume) == 0:
            import glob
            all_checkpoints = glob.glob(os.path.join(output_dir, 'checkpoint-*.pth'))
            latest_ckpt = -1
            for ckpt in all_checkpoints:
                t = os.path.basename(ckpt).split('-')[-1].split('.')[0]
                if t.isdigit():
                    latest_ckpt = max(int(t), latest_ckpt)
            if latest_ckpt >= 0:
                args.resume = os.path.join(output_dir, 'checkpoint-%d.pth' % latest_ckpt)

        # Se args.resume è una directory, cerca l'ultimo file .pth all'interno
        if args.resume and os.path.isdir(args.resume):
            import glob
            all_checkpoints = glob.glob(os.path.join(args.resume, 'checkpoint-*.pth'))
            latest_ckpt = -1
            for ckpt in all_checkpoints:
                t = os.path.basename(ckpt).split('-')[-1].split('.')[0]
                if t.isdigit():
                    latest_ckpt = max(int(t), latest_ckpt)
            if latest_ckpt >= 0:
                args.resume = os.path.join(args.resume, 'checkpoint-%d.pth' % latest_ckpt)

        if args.resume and os.path.isfile(args.resume):
            print("Auto resume checkpoint: %s" % args.resume)
            if args.enable_deepspeed:
                # Se per qualche motivo abilitassi deepspeed in futuro
                latest_ckpt = int(os.path.basename(args.resume).split('-')[-1].split('.')[0])
                _, client_states = model.load_checkpoint(args.output_dir, tag='checkpoint-%d' % latest_ckpt)
                args.start_epoch = client_states['epoch'] + 1
            else:
                # CARICAMENTO STANDARD PYTORCH (Per il tuo training attuale)
                checkpoint = torch.load(args.resume, map_location='cpu')
                model_without_ddp.load_state_dict(checkpoint['model'])
                print("Resume checkpoint %s" % args.resume)
                if 'optimizer' in checkpoint and 'epoch' in checkpoint:
                ...
                if model_ema is not None and 'model_ema' in checkpoint:
                    _load_checkpoint_for_ema(model_ema, checkpoint['model_ema'])
                print("With optim & sched (Standard PyTorch)!")
        else:
            print(f"No valid checkpoint found at {args.resume}, starting from scratch.")
...
def merge_batch_tensors_by_dict_key(batch):
    ...
    for tensor_key in batch[0]:
        values = [d[tensor_key] for d in batch]
        
        # Se tutti i valori sono None, passa None
        if all(v is None for v in values):
            batch_tensors[tensor_key] = None
        # Se batch misto (alcuni None, alcuni tensor) sostituisci None con zeri
        elif any(v is None for v in values):
            ref = next(v for v in values if v is not None)
            values = [v if v is not None else torch.zeros_like(ref) for v in values]
            batch_tensors[tensor_key] = torch.stack(values)
        elif isinstance(values[0], torch.Tensor):
            batch_tensors[tensor_key] = torch.stack(values)
        else:
            batch_tensors[tensor_key] = torch.tensor(values, dtype=torch.long)
