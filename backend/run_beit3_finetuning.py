...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='beit3_large_patch16_480_valence_arousal', type=str, metavar='MODEL',
                        help='Name of model to train')
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'va_regression'], # add 'va_regression' task
    ...
    parser.add_argument('--sentencepiece_model', type=str, default='./beit3.spm', 
    ...
    parser.add_argument('--loss_type', default='ccc', type=str, choices=['mse', 'ccc', 'robust', 'mse_ccc', 'robust_ccc'],
                        help='Loss function to use for VA regression')
    parser.add_argument('--test_file', default='global_test_conformal.jsonl', type=str, help='Name of the file for test/evaluation')
    parser.add_argument('--freeze_backbone', action='store_true', default=False,
                    help='Ablation: linear probing, freeze everything except valence_mlp/arousal_mlp')
    ...
    parser.add_argument('--finetune', 
                       default='/cfs/home/u036743/unilm/beit3/Multilingual_projection/VECMAP_RESUBMISSION/beit3_large_patch16_480_vqa_xlmr_projected.pth',
                       type=str, help='finetune from checkpoint')
...
def main(args, ds_init):
    ...
    if not args.model.endswith(args.task):
        # Add specific handling for Valence-Arousal (VA) Regression
    ...
    # --- ABLATION: linear probing ---
    if args.freeze_backbone:
        trainable_prefixes = ("valence_mlp", "arousal_mlp")
        ...
        for name, p in model.named_parameters():
            if name.startswith(trainable_prefixes):
                ...
                n_frozen += p.numel()
        print(f"Linear probing: {n_trainable:,} trainable params "
              f"(valence_mlp + arousal_mlp only), {n_frozen:,} frozen "
              f"(text_embed, all 24 encoder layers, and pooler).")
    ...
    mixup_fn = None # Mixup removed for regression

    if args.eval:
        data_loader_test = create_downstream_dataset(args, is_eval=True)
        test_stats, task_key = evaluate(data_loader_test, model, device, task_handler, args)
        exit(0)

    print(f"Start training for {args.epochs} epochs")
    start_time = time.time()

    # Initialize best RMSE tracker for checkpoint saving and model convergence monitoring.
    min_rmse = float('inf')
    patience = 2 # Maximum number of epochs without improvements
    patience_counter = 0 # Current tracker
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            current_performance = test_stats[task_key]
            print(f"Performance on val images: {current_performance:.4f} (Metric: {task_key})")

            # EARLY STOPPING LOGIC
            if current_performance < min_rmse:
                print(f"--- Improvement detected! Updating best model. ---")
                min_rmse = current_performance
            ...
            print(f'Min RMSE so far: {min_rmse:.4f}')

            # Update the log writer
            if log_writer is not None:
                log_writer.update(rmse=current_performance, head="perf", step=epoch)
        ...
        # FIX: Ensure this is aligned with the 'if data_loader_val' block
