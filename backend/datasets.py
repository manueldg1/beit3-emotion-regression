...
class HomogeneousBatchSampler(Sampler):
    def __init__(self, dataset, batch_size, num_replicas=1, rank=0, shuffle=True):
        ...
        for i, item in enumerate(dataset.items):
            ...
            has_img = bool(img_val) and img_val != "None"
            has_txt = bool(txt_val) and txt_val != "None"

            if has_img and has_txt:
                self.indices_multi.append(i)
            elif has_img:
                self.indices_img.append(i)
            elif has_txt:
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            random.shuffle(self.indices_multi)
    ...
    def __len__(self):
        return (len(self.indices_img) // self.batch_size) + \
               (len(self.indices_txt) // self.batch_size) + \
               (len(self.indices_multi) // self.batch_size)
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        tokenizer, num_max_bpe_tokens, task=None, args=None,
    ):
        ...
        # Get the list of .jsonl files for the current split (train, val, or test)
        ...
        files_to_load = [args.test_file] if (split == "test" and hasattr(args, 'test_file') and args.test_file) else index_files

        # Iterate through index files and load JSON entries
        for _index_file in files_to_load:
            index_file = os.path.join(data_path, _index_file)
            with open(index_file, mode="r", encoding="utf-8") as reader:
                for line in reader:
                    # json.loads preserves your "Text", "Image", "Valence", "Arousal" keys
        ...
        # Initialize special token IDs from tokenizer with safe defaults
        # Using getattr or 'or' logic ensures IDs are integers, not None
        self.bos_token_id = getattr(tokenizer, 'bos_token_id', None) or 0
        self.eos_token_id = getattr(tokenizer, 'eos_token_id', None) or 2
        self.pad_token_id = getattr(tokenizer, 'pad_token_id', None) or 1
        
        # Set remaining dataset attributes
        self.loader = default_loader
        self.transform = transform
        self.split = split
        
        # Print a small sample to verify keys
        if len(self.items) > 0:
            print(f"--- Dataset Initialized: Split {split} ---")
            print(f"--- Sample keys: {list(self.items[0].keys())} ---")
    ...
    def _get_image(self, image_path: str):
        ...
        try:
            ...
            print(f"[WARNING] Missing/corrupted image, skipping: {image_path} ({e})")
            return None  

    def _get_text_segment(self, text_segment, max_len=None):
        if isinstance(text_segment, str):
            # 1. Suddivide il testo in token (stringhe come 'hello', '##world')
            tokens = self.tokenizer.tokenize(text_segment)
            # 2. TRASFORMA LE STRINGHE IN ID (interi come 101, 2543, ecc.)
            tokens = self.tokenizer.convert_tokens_to_ids(tokens)
        else:
            tokens = text_segment[:]
            # Se la lista contiene già stringhe, convertile
        ...
        # Truncate to make room for BOS and EOS
        if len(tokens) > max_len - 2:
            tokens = tokens[:max_len - 2]

        # Now the concatenation is between integers only: [int] + [int, int...] + [int]
        ...
        # Return the list of numeric IDs padded with pad_token_id
    ...
    def __getitem__(self, index: int):
        ...
        if img_path and img_path != "None":
            ...
            if image is None:
                return None  # missing/corrupted image: item dropped by the collate fn
   
        text_content = item.get("Text")
        if text_content and text_content != "None":
            tokens, mask, _ = self._get_text_segment(text_content)
            text_description = torch.tensor(tokens, dtype=torch.long)
        ...
        return {
            ...
            "label":            label,
            # Metadata passed through collate for JSONL saving
...
class VARegressionDataset(BaseDataset):
    @staticmethod
    def get_index_files(split, task=None):
        # Mapping based on our file names
        if split == "train":
            return ["global_train_conformal.jsonl"]
        elif split == "val":
            return ["global_val_conformal.jsonl"]
        elif split == "test":
            # Per ora puntiamo al validation per la tua prova di "onestà" del modello
            return ["global_val_conformal.jsonl"]
        
        # Fallback for files cal1 and cal2
...
task2dataset = { 
    ...
    "va_regression": VARegressionDataset,
}


def custom_collate_fn(batch):
    # Drop items with a missing/corrupted image (returned as None by __getitem__)
    batch = [item for item in batch if item is not None]
    if len(batch) == 0:
        return None  # whole batch was dropped; the caller must skip this step

    labels = torch.stack([item['label'] for item in batch])
    # Images
    if batch[0]['image'] is None:
        assert all(item['image'] is None for item in batch), "Mixed batch detected"
        collated_images = None
    else:
        collated_images = torch.stack([item['image'] for item in batch])
    # Text
    if batch[0]['text_description'] is None:
        assert all(item['text_description'] is None for item in batch), "Mixed batch detected"
        collated_texts = None
        collated_masks = None
    else:
        collated_texts = torch.stack([item['text_description'] for item in batch])
        collated_masks = torch.stack([item['padding_mask'] for item in batch])


    # Metadata — passed as lists (not tensors), ignored by .to(device)
    # Used by eval_batch to save predictions + embeddings to JSONL
    metadata = {
        "Dataset":    [item.get("Dataset",    "unknown") for item in batch],
        "Language":   [item.get("Language",   "unknown") for item in batch],
        "Image_path": [item.get("Image_path", "")        for item in batch],
        "Text":       [item.get("Text",       "")        for item in batch],
        "Valence":    [item.get("Valence",    0.0)       for item in batch],
        "Arousal":    [item.get("Arousal",    0.0)       for item in batch],
    }

    return {
        "image":            collated_images,
        "text_description": collated_texts,
        "padding_mask":     collated_masks,
        "label":            labels,
        "metadata":         metadata,    # list of dicts, not a tensor
    }
...
def create_dataloader(dataset, is_train, batch_size, num_workers, pin_mem, dist_eval=False):
    ...
    if is_train:
        # Homogeneous batches only for training — ensures no mixed modality batches
        ...
        # For val/test: standard sequential sampler, no samples dropped
        # Still uses custom_collate_fn to handle None modalities correctly
        return torch.utils.data.DataLoader(
            dataset,
            batch_sampler=sampler,
            shuffle=False,
...
def create_dataset_by_split(args, split, is_train=True):
    ...
    dataset = dataset_class(
        ...
        task=args.task, args=args, **opt_kwargs, 
    )
    if is_train:
...
def create_downstream_dataset(args, is_eval=False):
    if is_eval:
        # Deve restituire SOLO il dataset, come faceva prima
